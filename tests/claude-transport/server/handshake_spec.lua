...
describe("WebSocket handshake", function()
	...
	describe("validate_upgrade_request", function()
		...
		it("rejects a cross-origin browser request", function()
			local req = make_request({ "Origin: https://evil.example.com" })
			local valid, err = handshake.validate_upgrade_request(req, nil)
			assert.is_false(valid)
			assert.matches("[Oo]rigin", err)
		end)

		it("rejects a null origin", function()
			local req = make_request({ "Origin: null" })
			local valid = handshake.validate_upgrade_request(req, nil)
			assert.is_false(valid)
		end)

		it("accepts a localhost origin", function()
			local req = make_request({ "Origin: http://localhost:8080" })
			local valid = handshake.validate_upgrade_request(req, nil)
			assert.is_true(valid)
		end)

		it("accepts a 127.0.0.1 origin", function()
			local req = make_request({ "Origin: http://127.0.0.1" })
			local valid = handshake.validate_upgrade_request(req, nil)
			assert.is_true(valid)
		end)

		it("accepts a request without an Origin header", function()
			local valid = handshake.validate_upgrade_request(valid_request, nil)
			assert.is_true(valid)
		end)
