---@brief Centralized logger for Claude Transport Neovim integration.
-- Provides level-based logging.
---@module 'claude-transport.logger'
local M = {}
...
---@param plugin_config ClaudeTransportConfig The configuration table from claude-transport.init.state.config.
function M.setup(plugin_config)
	...
	if conf and conf.log_level and level_values[conf.log_level] then
		...
		vim.notify(
			"ClaudeTransport Logger: Invalid or missing log_level in configuration (received: "
...
local function log(level, component, message_parts)
	...
	vim.schedule(function()
		...
		elseif level == M.levels.WARN then
			vim.notify(prefix .. " " .. message, vim.log.levels.WARN, { title = "ClaudeTransport Warning" })
		else
